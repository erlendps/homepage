...
router.post("/newuser", async (req, res) => {
  logger.log(
    "info",
    `HTTP ${res.statusCode} ${req.method} /api/admin${req.url}`
  );
...
router.get("/projects", async (req, res) => {
  ...
  if (result === 500) {
    return res
      .status(500)
      .send(
        "<h1>500 Internal Server Error</h1><p>Something is wrong on our end, sorry!</p>"
      );
  }
...
router.post(
  "/newproject",
  project_upload.single("project_image"),
  async (req, res) => {
    let body = req.body;
    logger.log(
      "info",
...
router.post(
  "/cookbook/new_recipe",
  cookbook_upload.single("recipe_image"),
  async (req, res) => {
    logger.log(
      "info",
      `HTTP ${res.statusCode} ${req.method} /api/admin${req.url}`
