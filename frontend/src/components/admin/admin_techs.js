  ...
  render() {
    return (
      <div className="new-tech" onSubmit={this.handleSubmit}>
        <h2 className="mt--5">Add a new tech</h2>
        <form className="new-tech-form">
          <input type="text" value={this.state.value} onChange={this.handleChange} placeholder="Name" className="form-input" required/>
        ...
        {props.techs.map((tech) => {
          return (
            <li key={tech.techID} className="tech-list-item">
              {tech.name}
              <Delete section="tech" id={tech.techID} notifyParent={props.forceRerenderOfAllTechs}/>
            </li>);
        })}
      </ul>
    </div>
    ...
    return (
      <div className="admin-techs-page">
        ...
        <div className="admin-techs-content">
          ...
          <TechForm forceRerenderOfAllTechs={forceStateChange} togglePopup={togglePopup} />
        </div>
        {popup && 
